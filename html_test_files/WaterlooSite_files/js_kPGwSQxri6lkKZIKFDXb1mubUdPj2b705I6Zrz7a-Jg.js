...
(function ($) {
  ...
  Drupal.behaviors.responsiveMenuCombined = {
    attach: function (context, settings) {
      // Make sure we're at the top of the menu when something is clicked.
      $('.responsive-nav-menu .sub-nav-check', context).on('click', function () {
        $('html, body').animate({scrollTop : 0}, 'fast');
      });
      // Tell the checked menu that it's the current menu.
      $('.responsive-nav-menu input[name=tab-group-responsive_menu_combined]:checked', context).data('current','true');
      // When a menu tab is clicked, if it’s already the currently displayed menu, go to the root of that menu.
      $('.responsive-nav-menu input[name=tab-group-responsive_menu_combined]', context).on('click', function () {
        if ($(this).data('current') == 'true') {
          // Go to the root of the menu by unchecking any sub-navigation in this tab.
          $('.sub-nav-check:checked', $(this).closest('.tab')).prop('checked', false);
        }
        else {
          // Update the current menu.
          $('.responsive-nav-menu input[name=tab-group-responsive_menu_combined]', context).data('current', '');
          $(this).data('current', 'true');
        }
      });
      $('button.toggle', context).on('click', function () {
  ...
  Drupal.behaviors.uw_ct_embedded_facts_and_figures = {
    ...
    attach: function (context, settings) {
      ...
      $(window).ready(function () {
        ...
        $('.content div[class*="highlighted-fact-wrapper"]').each(function (index, value) {

          // If we have a carousel in the FF, add the Owl Carousel.
          if ($(this).attr("data-use-carousel") == "Yes") {

            // Add the Owl Carousel on the FF in the body.
            ($(this)).owlCarousel({
